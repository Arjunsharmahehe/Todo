...
export default function TaskList() {
  ...
  return (
    <ul className="flex flex-col gap-3 px-3 py-2 items-center border-2 w-5/6 sm:4/5 md:w-3/4 lg:w-1/2 border-neutral-800 rounded-md justify-between">
      {tasks?.length > 0 ? tasks.map((task) => (
        <li key={task.id} className="flex flex-wrap gap-3 px-3 py-2 items-center w-full border-2 border-neutral-800 rounded-md justify-between">
